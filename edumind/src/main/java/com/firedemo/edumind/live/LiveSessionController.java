...
import com.firedemo.edumind.live.service.LiveSessionService;
import com.firedemo.edumind.live.service.LiveConfusionService;
import com.firedemo.edumind.live.service.ReportService;
import com.firedemo.edumind.live.service.StudentPresenceService;
import com.firedemo.edumind.live.security.ClassroomStudentPrincipal;
import com.firedemo.edumind.live.security.StudentDeviceTokenService;
...
public class LiveSessionController {
    ...
    private final LiveConfusionService confusionService;
    ...
    public Result<Map<String, Object>> markConfusion(@RequestBody Map<String, Object> body) {
        ...
        if (interactionId != null) {
            LiveConfusionEvent existing = confusionService.findExisting(interactionId, studentId);
            if (existing != null) {
        ...
        // 获取题目信息
        Interaction interaction = interactionId != null ? interactionService.getById(interactionId) : null;
        ...
        // 记录事件
        confusionService.record(LiveConfusionEvent.builder()
                .sessionId(sessionId)
    ...
    private String generateExplanation(String knowledgePoint, String questionText,
        ...
        String raw = agentService.chat(prompt.toString(), "1");
    ...
    public Result<Map<String, Object>> getConfusionStats(@PathVariable Long sessionId) {
        LiveConfusionService.SessionSummary summary = confusionService.summarizeSession(sessionId);
        return Result.success(Map.of(
                "stats", summary.stats(),
