...
import com.firedemo.edumind.platform.ratelimit.RateLimit.Dimension;
import com.firedemo.edumind.platform.ratelimit.RateLimit.TimeUnit;
...
public class ChatController {
    ...
    public ChatController(AgentService agentService,
        ...
        this(agentService, fileStorageService, chatHistoryService, userService,
                classService, visualAssetService, visionUnderstandingService,
                objectMapper, executionContextFactory, null, null, new AgentUiEventBus());
    }

    @GetMapping("/health")
    public ResponseEntity<Map<String, Object>> health() {
        boolean llmHealthy = agentService.checkConnection();
    ...
    public ResponseEntity<StreamingResponseBody> streamMessage(@RequestBody ChatStreamRequest request) {
        ...
        agentService.registerSessionContext(context);

        String agentMessage = addTrustedTeachingContext(message, scope);
        Flux<String> stream = agentService.streamChat(agentMessage, context, null);
    ...
    public ResponseEntity<Map<String, Object>> multimodalMessage(
        ...
        String answer = agentService.chat(addTrustedTeachingContext(agentMessage, scope), context, null);
    ...
    public ResponseEntity<StreamingResponseBody> multimodalStreamMessage(
        ...
        Flux<String> stream = agentService.streamChat(addTrustedTeachingContext(agentMessage, scope), context, null);
    ...
    private Long resolveCourseId(Long userId) {
        ...
        try {
            ...
            if (user != null && user.getClassId() != null) {
                ClassInfo cls = classService.getClassById(user.getClassId());
