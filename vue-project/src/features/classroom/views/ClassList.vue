...
import {
  ...
  type Course,
  type PresetTemplate,
} from '@/features/classroom/api/course'
...
async function fetchClasses() {
  ...
  try {
    ...
    if (res.data.code === 200) {
      ...
      classList.value = groups.flatMap((g) =>
        g.classes.map((c) => ({
          ...c,
...
function onPresetChange(key: string) {
  ...
  if (p) {
    courseForm.value.systemPrompt = p.prompt.replace(
      '{{courseName}}',
      courseForm.value.name || '新课程',
    )
  }
}
  ...
  try {
    if (editingCourse.value && f.id) {
      ...
      await apiCreateCourse({
        name: f.name.trim(),
        presetKey: f.presetKey || undefined,
        systemPrompt: f.systemPrompt || undefined,
        knowledgeScope: f.knowledgeScope || undefined,
      })
...
async function handleDeleteCourse() {
  if (
    !confirm(
      `确定要删除课程「${courseForm.value.name}」吗？\n课程下的班级不会删除，但会变成未分组。`,
    )
  )
    return
  try {
...
function openCreateDialogForGroup(group: CourseGroup) {
  openCreateDialog()
  createForm.value.courseGroup = group.name
  createForm.value.courseId = group.courseId
}
...
async function createClass() {
  ...
  try {
    ...
    ElMessage.error(getApiErrorMessage(requestError, '创建失败'))
  } finally {
  ...
  return courseGroups.value
    .map((g) => ({
      ...
      classes: g.classes.filter(
        (c) => c.name.toLowerCase().includes(kw) || g.name.toLowerCase().includes(kw),
      ),
    }))
...
<template>
  <div class="class-list-page">
    <div class="page-header">
      <div class="header-left">
        ...
        <span class="header-sub"
          >{{ courseGroups.length }} 门课程 / {{ classList.length }} 个班级</span
        >
      </div>
    ...
    <div v-else-if="error" class="empty-state">
      ...
      <p>{{ error }}</p>
      <button class="btn-retry" @click="fetchClasses">重试</button>
    </div>
    <div v-else-if="classList.length === 0" class="empty-state">
      <div class="empty-icon">📭</div>
      <p>暂无班级</p>
      <p class="empty-sub">点击「创建课程」或「创建班级」开始管理你的教学班级</p>
    </div>
      ...
      <section class="course-section">
        <div class="course-header">
          <div class="course-info">
            <div class="course-icon">{{ group.name === '未分组班级' ? '📦' : '📘' }}</div>
            <div class="course-meta">
              <h3 class="course-name">{{ group.name }}</h3>
              <span class="course-stats"
                >{{ group.classes.length }} 个班级<template v-if="group.activeCount > 0">
                  · {{ group.activeCount }} 个进行中</template
                >
                · {{ group.totalStudents }} 名学生</span
              >
            </div>
          </div>
          <button
            v-if="group.name !== '未分组班级' && group.courseId"
            class="btn-edit-course"
            @click="openEditCourseDialog(group)"
            title="编辑课程"
          >
            ⚙️ 课程设置
          </button>
        </div>
        <div v-if="group.classes.length === 0" class="empty-course">
          <p>这门课程下还没有班级</p>
          <button class="btn-add-class" @click="openCreateDialogForGroup(group)">+ 添加班级</button>
        </div>
        <div v-else class="class-grid">
          <div
            ...
            :key="cls.id"
            class="class-card"
            :class="{ archived: cls.status === 'ARCHIVED' }"
          >
            <div class="card-header">
              <div class="card-avatar" :class="{ 'archived-avatar': cls.status === 'ARCHIVED' }">
                {{ cls.name.charAt(0) }}
              </div>
              <div class="card-title-group">
                <div class="card-title-row">
                  ...
                  <span
                    class="status-dot"
                    :class="cls.status === 'ACTIVE' ? 'active' : 'archived'"
                    >{{ cls.status === 'ACTIVE' ? '进行中' : '已归档' }}</span
                  >
                </div>
                <span class="card-date">创建于 {{ formatDate(cls.createdAt) }}</span>
              </div>
            </div>
            ...
            <div class="card-stats">
              <div class="stat">
                ...
                ><span class="stat-label">位学生</span>
              </div>
              <div class="stat invite-stat" @click.stop="openInvite(cls.inviteCode)">
                <span class="stat-icon">🔗</span
                ><span class="stat-value code">{{ cls.inviteCode }}</span
                ><span class="stat-label invite-hint">点击邀请</span>
              </div>
            </div>
            <button
              class="btn-manage"
              :class="{ 'btn-ghost': cls.status === 'ARCHIVED' }"
              @click="goManage(cls.id)"
            >
              {{ cls.status === 'ACTIVE' ? '管理班级 →' : '查看详情 →' }}
            </button>
          </div>
        </div>
      </section>
    ...
    <div v-if="showCreateCourseDialog" class="dialog-overlay" @click.self="closeCreateCourseDialog">
      <div class="dialog-content dialog-md">
        <div class="dialog-header">
          <h3>{{ editingCourse ? '⚙️ 课程设置' : '📘 创建新课程' }}</h3>
          <button class="dialog-close" @click="closeCreateCourseDialog">×</button>
        </div>
        <div class="dialog-body">
          <div class="form-group">
            ...
            ><input
              ...
              class="form-input"
              maxlength="30"
              @keyup.enter="handleCreateOrEditCourse"
            />
          </div>
          <div v-if="!editingCourse" class="form-group">
            <label>预设模板</label>
            <select
              v-model="courseForm.presetKey"
              @change="onPresetChange(courseForm.presetKey)"
              class="form-select"
            >
          ...
          <div class="form-group">
            ...
            <textarea
              v-model="courseForm.systemPrompt"
              rows="6"
              class="form-textarea"
              placeholder="描述 AI 助教在该课程中的行为规则和知识范围..."
            ></textarea>
          </div>
        ...
        <div class="dialog-footer">
          <button class="btn-cancel" @click="closeCreateCourseDialog">取消</button
          ><button
            class="btn-primary"
            @click="handleCreateOrEditCourse"
            :disabled="!courseForm.name.trim() || creatingCourse"
          >
            {{ creatingCourse ? '保存中...' : editingCourse ? '保存' : '确认创建' }}
          </button>
        </div>
      </div>
    </div>
      ...
      <div class="dialog-content">
        ...
        <div class="dialog-body">
          ...
          <div class="form-group">
            <label>QQ群号</label
            ><input
              v-model="createForm.qqGroupId"
              placeholder="选填，用于 AI 自动识别班级"
              class="form-input"
              maxlength="20"
            />
          </div>
          <div class="form-group">
            ...
            ><textarea
              ...
              placeholder="选填，例如：2024-2025学年第一学期"
              rows="2"
              class="form-textarea"
              maxlength="200"
            ></textarea>
