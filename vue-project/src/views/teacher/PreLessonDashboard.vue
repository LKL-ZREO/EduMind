  ...
  <div class="pre-lesson-page">
    ...
    <template v-else-if="data">
      ...
      <section class="overview-cards">
        <div class="card">
          <span class="card-val">{{ data.totalStudents }}</span
          ><span class="card-label">学生总数</span>
        </div>
        <div class="card">
          <span class="card-val">{{ data.avgScore }}%</span><span class="card-label">作业均分</span>
        </div>
        <div class="card warn">
          <span class="card-val">{{ data.warningCount }}</span
          ><span class="card-label">需关注学生</span>
        </div>
        <div class="card">
          <span class="card-val">{{ data.liveSessionCount }}</span
          ><span class="card-label">历史课堂</span>
        </div>
        <div class="card">
          <span class="card-val">{{ data.liveAvgCorrectRate }}%</span
          ><span class="card-label">互动正确率</span>
        </div>
        <div class="card">
          <span class="card-val">{{ data.participationRate }}%</span
          ><span class="card-label">参与率</span>
        </div>
      </section>
      ...
      <div class="main-grid">
        ...
        <section class="panel calendar-panel">
          <div class="calendar-header">
            <h3>📅 教学日历</h3>
            <el-button size="small" type="primary" @click="openCalendarPlanDialog"
              >+ 添加计划</el-button
            >
          </div>
            ...
            <div v-for="w in timeline.weeks" :key="w.weekNumber" class="cal-week">
              ...
              >
                ...
                <div class="cal-body">
                  ...
                  <div class="cal-meta">
                    <span v-if="t.date" class="cal-date"
                      >{{ t.date }}{{ t.time ? ' ' + t.time : '' }}</span
                    >
                    <span v-if="t.status" class="cal-status" :class="statusClass(t.status)">{{
                      statusLabel(t)
                    }}</span>
      ...
      <section class="actions">
        ...
        <button
          class="btn-primary"
          @click="$router.push('/teacher/preview/create?classId=' + selectedClass)"
        >
          📖 发布预习任务
        </button>
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="planGenerating" @click="generatePlan"
          >🤖 生成教案</el-button
        >
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="planDialogOpen" title="📅 添加教学计划" width="420px">
      <el-form label-width="80px">
        <el-form-item label="主题"
          ><el-input v-model="planForm.topic" placeholder="如：结构体与联合体"
        /></el-form-item>
        <el-form-item label="知识点"
          ><el-input v-model="planForm.knowledgePoints" placeholder="如：struct, union, typedef"
        /></el-form-item>
        <el-form-item label="计划日期"
          ><el-input v-model="planForm.plannedDate" type="date"
        /></el-form-item>
      </el-form>
      <template #footer>
...
async function loadClasses() {
  try {
    ...
    if (classList.value.length && !selectedClass.value) {
      selectedClass.value = classList.value[0]?.id ?? null
      load()
    }
  } catch {
    /* ignore */
...
async function loadAiSuggestion() {
  ...
  try {
    ...
    if (data.value && suggestion) {
      data.value = { ...data.value, aiSuggestion: suggestion }
    }
  } catch {
    /* ignore */
  } finally {
...
async function generatePlan() {
  ...
  try {
    const weakPoints = data.value?.weakPoints?.map((weakPoint) => weakPoint.name) || []
...
function statusLabel(t: WeekItem) {
...
.cal-item.cal-session {
  border-left-color: #409eff;
}
.cal-item.cal-homework {
