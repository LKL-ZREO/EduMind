  ...
  <div class="tasks-page">
    ...
    <section class="create-section">
      ...
      <div v-if="showCreateForm" class="create-form">
        ...
        <div class="form-group">
          <label>作业说明</label>
          <RichTextEditor v-model="createForm.description" placeholder="请输入作业要求、题目说明..." />
        </div>
    ...
    <div v-if="showEditModal" class="modal-overlay" @click.self="closeEditModal">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <label>作业说明</label>
            <RichTextEditor v-model="editForm.description" placeholder="请输入作业要求、题目说明..." />
          </div>
          <div class="form-actions">
...
export default {
  ...
  setup() {
    ...
    return {
      classStore,
      classList: computed(() => classStore.classList)
    }
  },
  data() {
    return {
 ...
 async mounted() {
  ...
  methods: {
      ...
      try {
        const results = await Promise.allSettled(
          this.classList.map(cls =>
            request.get(`/tasks?classId=${cls.id}`).then(r => r.data)
          )
        )
        this.tasks = results
    ...
    async saveEdit() {
      ...
      this.submitting = true
      try {
        const res = await request.put(`/tasks/${this.editingTaskId}`, {
    ...
    async testReminder(task) {
      ...
      try {
        const res = await request.post(`/tasks/${task.id}/test-reminder`)
        if (res.data.code === 200) {
          ElMessage.success('测试提醒已发送！' + (res.data.data?.message || ''))
        } else {
